...
This file records the main product and code changes for Anchor.

## v3.1 - Remove Google Calendar Support

- Removed Google Calendar OAuth, multi-account connection, token encryption and refresh, Calendar API event sync, and Google-specific settings.
- Kept iCloud/CalDAV calendar discovery, visibility controls, caching, and event create/edit/delete workflows unchanged.
- Added startup cleanup for legacy Google accounts, calendar sources, and locally stored Google secrets.
- Google Account authorization must be revoked manually from the user's third-party connections page.

## v3.0 - Unified Calendars

- Finalized the Anchor project identity across plugin metadata, internal identifiers, UI text, documentation, and local test packaging.
- Added unified Today and Next 7 Days views across every enabled iCloud/CalDAV and Google calendar.
- Added a dedicated Anchor Calendar tab with switchable Month, Week, and Day views, an iCloud/Google source sidebar, date and time-slot event creation, and read-only event details.
...
- Made Anchor render immediately while todos, workouts, and remote calendars load independently in the background.
...
- Added iCloud/CalDAV calendar sync for the Anchor dashboard.
...
- Saved routine changes back into `Anchor/Workout Plan.md`.
...
- Added the `Anchor` custom dashboard view.
- Added startup auto-open behavior.
- Added the first dashboard layout with Schedule, Workout, and Todo Manager panels.
- Added Markdown-backed todo storage in `Anchor/Todo Inbox.md`.
- Added Markdown-backed workout storage in `Anchor/Workout Plan.md` and `Anchor/Workout Log.md`.
